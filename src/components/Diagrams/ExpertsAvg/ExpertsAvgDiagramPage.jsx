...
const ExpertsAvgDiagramPage = () => {
    const { expertMarks, startKef, experts } = useContext(DataContext);

    // Дані для діаграми
    let weight = experts.reduce((sum,val)=>{return sum + val.weight},0)/4;
    let marks = expertMarks.map((val) => val.reduce((sum,v)=>{return sum + v},0)/val.length);
    let kef = startKef.map((val) => val.reduce((sum,v)=>{return sum + v},0)/val.length);
  ...
  return (
    <div>
      <h1>Діаграма середніх показників</h1>
...
export default ExpertsAvgDiagramPage;