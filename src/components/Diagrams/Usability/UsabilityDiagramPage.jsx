...
const UsabilityDiagramPage = () => {
    const { expertMarks, startKef, experts } = useContext(DataContext);

    // Дані для діаграми
    let weight = experts[1].weight;
    let marks = expertMarks.map((val) => val[1]);
    let kef = startKef.map((val) => val[1]);
...
export default UsabilityDiagramPage;